        ...
        try {
            switch ($_POST['action']) {
                case 'create':
                    ...
                    $_SESSION['alert_message'] = 'Data mekanik berhasil ditambahkan!';
                ...
                case 'update':
                    ...
                    $_SESSION['alert_message'] = 'Data mekanik berhasil diupdate!';
                    $_SESSION['alert_type'] = 'success';
                    $_SESSION['alert_title'] = 'Sukses!';
                    $_SESSION['alert_icon'] = 'success';
                    break;
                ...
                case 'delete':
                    ...
                    $_SESSION['alert_message'] = 'Data mekanik berhasil dihapus!';
                    $_SESSION['alert_type'] = 'success';
                    $_SESSION['alert_title'] = 'Sukses!';
                    $_SESSION['alert_icon'] = 'success';
                    break;
...
// Fetch all mechanics
$stmt = $pdo->prepare("SELECT * FROM tb_user WHERE role = 'mekanik' ORDER BY created_at DESC");
$stmt->execute();
$mekaniks = $stmt->fetchAll();
?>
<!doctype html>
...
<head>
    <title>Mekanik Management | Datta Able Dashboard Template</title>
    <!-- [Meta] -->
    <meta charset="utf-8" />
...
<body>
            ...
            <div class="page-header">
                <div class="page-block">
                    <div class="page-header-title">
                        <h5 class="mb-0 font-medium">Mekanik Management</h5>
                    </div>
                    <ul class="breadcrumb">
                ...
                <div class="col-span-12">
                    <div class="card">
                        <div class="card-header">
                            <h5 class="card-title">Data Mekanik</h5>
                            <button class="btn btn-primary" onclick="openCreateModal()">
                                <i class="fas fa-plus"></i> Tambah Mekanik
                            </button>
                        </div>
                        <div class="card-body">
                            <table id="mekanikTable" class="display" style="width:100%">
                                <thead>
                                    <tr>
                                        <th>No</th>
                                        <th>Foto</th>
                                ...
                                <tbody>
                                    <?php foreach ($mekaniks as $index => $mekanik): ?>
                                        <tr>
                                            <td><?= $index + 1 ?></td>
                                            <td>
                                                <img src="<?= $mekanik['photo_profile'] ? '../../assets/images/profile/' . $mekanik['photo_profile'] : '../../assets/images/profile/avatar-2.jpg' ?>"
                                                    alt="Avatar" class="avatar">
                                            </td>
                                            <td><?= htmlspecialchars($mekanik['nama']) ?></td>
                                            <td><?= htmlspecialchars($mekanik['nohp']) ?></td>
                                            <td><?= htmlspecialchars($mekanik['email']) ?></td>
                                            <td><span class="badge bg-info"><?= ucfirst($mekanik['role']) ?></span></td>
                                            <td><?= date('d/m/Y H:i', strtotime($mekanik['created_at'])) ?></td>
                                            <td>
                                                <button class="btn btn-success btn-sm" onclick="viewMekanik(<?= $mekanik['id_user'] ?>)">
                                                    <i class="fas fa-eye"></i>
                                                </button>
                                                <button class="btn btn-warning btn-sm" onclick="editMekanik(<?= $mekanik['id_user'] ?>)">
                                                    <i class="fas fa-edit"></i>
                                                </button>
                                                <button class="btn btn-danger btn-sm" onclick="deleteMekanik(<?= $mekanik['id_user'] ?>)">
    ...
    <!-- Enhanced Create/Edit Modal - Ganti bagian modal yang ada -->
    <div id="mekanikModal" class="modal-overlay">
        <div class="modal-content">
            <div class="modal-header">
                <h4 class="modal-title" id="modalTitle">Tambah Mekanik</h4>
                <button type="button" class="close" onclick="closeModal()">
                    <i class="fas fa-times"></i>
                </button>
            </div>
            <div class="modal-body">
                <form id="mekanikForm" enctype="multipart/form-data" method="POST">
                    <input type="hidden" name="action" id="formAction" value="create">
                    <input type="hidden" name="id_user" id="mekanikId">
            ...
            <div class="modal-footer">
                ...
                <button type="submit" form="mekanikForm" class="btn btn-primary" id="submitBtn">
    ...
    <!-- Enhanced View Modal - Ganti bagian view modal yang ada -->
    <div id="viewModal" class="modal-overlay">
        <div class="modal-content">
            <div class="modal-header">
                <h4 class="modal-title">
                    <i class="fas fa-eye"></i> Detail Mekanik
                </h4>
    ...
    <script>
        ...
        // Mekanik data for JavaScript
        const mekaniks = <?= json_encode($mekaniks) ?>;

        // Open create modal
        function openCreateModal() {
            document.getElementById('modalTitle').innerHTML = '<i class="fas fa-plus"></i> Tambah Mekanik';
            ...
            const modal = document.getElementById('mekanikModal');
        ...
        // View mekanik
        function viewMekanik(id) {
            const mekanik = mekaniks.find(m => m.id_user == id);
            if (mekanik) {
                const photoSrc = mekanik.photo_profile ?
                    '../../assets/images/profile/' + mekanik.photo_profile :
            ...
            <div class="view-profile-section">
                ...
                <div class="view-profile-name">${mekanik.nama}</div>
                <div class="view-profile-role">${mekanik.role.charAt(0).toUpperCase() + mekanik.role.slice(1)}</div>
            </div>
            <div class="view-details">
                <div class="detail-item">
                    ...
                    <div class="detail-content">
                        ...
                        <div class="detail-value">${mekanik.nohp}</div>
                    </div>
                </div>
                <div class="detail-item">
                    <div class="detail-icon">
                    ...
                    <div class="detail-content">
                        ...
                        <div class="detail-value">${mekanik.email}</div>
                    </div>
                </div>
                <div class="detail-item">
                    <div class="detail-icon">
                    ...
                    <div class="detail-content">
                        ...
                        <div class="detail-value">${new Date(mekanik.created_at).toLocaleDateString('id-ID', {
                            year: 'numeric',
                            month: 'long',
                            day: 'numeric',
                            hour: '2-digit',
                ...
                <div class="detail-item">
                    ...
                    <div class="detail-content">
                        ...
                        <div class="detail-value">${new Date(mekanik.updated_at).toLocaleDateString('id-ID', {
                            year: 'numeric',
                            month: 'long',
                            day: 'numeric',
                            hour: '2-digit',
        ...
        function editMekanik(id) {
            const mekanik = mekaniks.find(m => m.id_user == id);
            if (mekanik) {
                document.getElementById('modalTitle').innerHTML = '<i class="fas fa-edit"></i> Edit Mekanik';
                document.getElementById('formAction').value = 'update';
                document.getElementById('submitBtn').innerHTML = '<i class="fas fa-save"></i> Update';
                document.getElementById('mekanikId').value = mekanik.id_user;
                document.getElementById('oldPhoto').value = mekanik.photo_profile || '';
                document.getElementById('nama').value = mekanik.nama;
                document.getElementById('nohp').value = mekanik.nohp;
                document.getElementById('email').value = mekanik.email;
                document.getElementById('password').value = mekanik.password;

                // Show current photo
                const preview = document.getElementById('imagePreview');
                const label = document.querySelector('label[for="photo_profile"]');
                if (mekanik.photo_profile) {
                    preview.src = '../../assets/images/profile/' + mekanik.photo_profile;
                    preview.classList.remove('hidden');
        ...
        function deleteMekanik(id) {
            ...
            <div style="text-align: center; margin: 20px 0;">
                ...
                <p style="font-size: 1.1rem; margin-bottom: 10px;">Apakah Anda yakin ingin menghapus mekanik:</p>
                <p style="font-size: 1.3rem; font-weight: 600; color: #333;">${mekanik ? mekanik.nama : 'ini'}</p>
        ...
        document.addEventListener('DOMContentLoaded', function() {
            // Close modal when clicking outside
            document.getElementById('mekanikModal').addEventListener('click', function(e) {
                if (e.target === this) {
                    closeModal();
                }
            });
