        ...
        try {
            switch ($_POST['action']) {
                case 'create':
                    ...
                    $_SESSION['alert_message'] = 'Data kasir berhasil ditambahkan!';
                ...
                case 'update':
                    ...
                    $_SESSION['alert_message'] = 'Data kasir berhasil diupdate!';
                    $_SESSION['alert_type'] = 'success';
                    $_SESSION['alert_title'] = 'Sukses!';
                    $_SESSION['alert_icon'] = 'success';
                    break;
                ...
                case 'delete':
                    ...
                    $_SESSION['alert_message'] = 'Data kasir berhasil dihapus!';
                    $_SESSION['alert_type'] = 'success';
                    $_SESSION['alert_title'] = 'Sukses!';
                    $_SESSION['alert_icon'] = 'success';
                    break;
...
// Fetch all kasir
$stmt = $pdo->prepare("SELECT * FROM tb_user WHERE role = 'kasir' ORDER BY created_at DESC");
$stmt->execute();
$kasir_data = $stmt->fetchAll();
?>
<!doctype html>
...
<head>
    <title>Kasir Management | Datta Able Dashboard Template</title>
    <!-- [Meta] -->
    <meta charset="utf-8" />
...
<body>
            ...
            <div class="page-header">
                <div class="page-block">
                    <div class="page-header-title">
                        <h5 class="mb-0 font-medium">Kasir Management</h5>
                    </div>
                    <ul class="breadcrumb">
                        <li class="breadcrumb-item" aria-current="page">Master Data</li>
                        <li class="breadcrumb-item" aria-current="page">Data User</li>
                        <li class="breadcrumb-item"><a href="kasir.php">Kasir</a></li>
                    </ul>
                </div>
            </div>
            <!-- [ breadcrumb ] end -->
            ...
            <div class="grid grid-cols-12 gap-x-6">
                <div class="col-span-12">
                    <div class="card">
                        ...
                        <div class="card-body">
                            <table id="kasirTable" class="display" style="width:100%">
                                <thead>
                                    <tr>
                                        <th>No</th>
                                        <th>Foto</th>
                                ...
                                <tbody>
                                    <?php foreach ($kasir_data as $index => $kasir): ?>
                                        <tr>
                                            <td><?= $index + 1 ?></td>
                                            <td>
                                                <img src="<?= $kasir['photo_profile'] ? '../../assets/images/profile/' . $kasir['photo_profile'] : '../../assets/images/profile/avatar-2.jpg' ?>"
                                                    alt="Avatar" class="avatar">
                                            </td>
                                            <td><?= htmlspecialchars($kasir['nama']) ?></td>
                                            <td><?= htmlspecialchars($kasir['nohp']) ?></td>
                                            <td><?= htmlspecialchars($kasir['email']) ?></td>
                                            <td><span class="badge bg-info"><?= ucfirst($kasir['role']) ?></span></td>
                                            <td><?= date('d/m/Y H:i', strtotime($kasir['created_at'])) ?></td>
                                            <td>
                                                <button class="btn btn-success btn-sm" onclick="viewKasir(<?= $kasir['id_user'] ?>)">
                                                    <i class="fas fa-eye"></i>
                                                </button>
                                                <button class="btn btn-warning btn-sm" onclick="editKasir(<?= $kasir['id_user'] ?>)">
                                                    <i class="fas fa-edit"></i>
                                                </button>
                                                <button class="btn btn-danger btn-sm" onclick="deleteKasir(<?= $kasir['id_user'] ?>)">
    ...
    <div id="kasirModal" class="modal-overlay">
        <div class="modal-content">
            <div class="modal-header">
                <h4 class="modal-title" id="modalTitle">Tambah Kasir</h4>
                <button type="button" class="close" onclick="closeModal()">
                    <i class="fas fa-times"></i>
                </button>
            </div>
            <div class="modal-body">
                <form id="kasirForm" enctype="multipart/form-data" method="POST">
                    <input type="hidden" name="action" id="formAction" value="create">
                    <input type="hidden" name="id_user" id="kasirId">
            ...
            <div class="modal-footer">
                ...
                <button type="submit" form="kasirForm" class="btn btn-primary" id="submitBtn">
        ...
        <div class="modal-content">
            <div class="modal-header">
                <h4 class="modal-title">
                    <i class="fas fa-eye"></i> Detail Kasir
                </h4>
    ...
    <script>
        ...
        // Kasir data for JavaScript
        const kasir_data = <?= json_encode($kasir_data) ?>;

        // Open create modal
        function openCreateModal() {
            document.getElementById('modalTitle').innerHTML = '<i class="fas fa-plus"></i> Tambah Kasir';
            ...
            const modal = document.getElementById('kasirModal');
        ...
        // View kasir
        function viewKasir(id) {
            const kasir = kasir_data.find(k => k.id_user == id);
            if (kasir) {
                const photoSrc = kasir.photo_profile ?
                    '../../assets/images/profile/' + kasir.photo_profile :
            ...
            <div class="view-profile-section">
                ...
                <div class="view-profile-name">${kasir.nama}</div>
                <div class="view-profile-role">${kasir.role.charAt(0).toUpperCase() + kasir.role.slice(1)}</div>
            </div>
            <div class="view-details">
                <div class="detail-item">
                    ...
                    <div class="detail-content">
                        ...
                        <div class="detail-value">${kasir.nohp}</div>
                    </div>
                </div>
                <div class="detail-item">
                    <div class="detail-icon">
                    ...
                    <div class="detail-content">
                        ...
                        <div class="detail-value">${kasir.email}</div>
                    </div>
                </div>
                <div class="detail-item">
                    <div class="detail-icon">
                    ...
                    <div class="detail-content">
                        ...
                        <div class="detail-value">${new Date(kasir.created_at).toLocaleDateString('id-ID', {
                            year: 'numeric',
                            month: 'long',
                            day: 'numeric',
                            hour: '2-digit',
                ...
                <div class="detail-item">
                    ...
                    <div class="detail-content">
                        ...
                        <div class="detail-value">${new Date(kasir.updated_at).toLocaleDateString('id-ID', {
                            year: 'numeric',
                            month: 'long',
                            day: 'numeric',
                            hour: '2-digit',
        ...
        function editKasir(id) {
            ...
            if (kasir) {
                ...
                document.getElementById('kasirId').value = kasir.id_user;
                document.getElementById('oldPhoto').value = kasir.photo_profile || '';
                document.getElementById('nama').value = kasir.nama;
                document.getElementById('nohp').value = kasir.nohp;
                document.getElementById('email').value = kasir.email;
                document.getElementById('password').value = kasir.password;

                // Show current photo
                const preview = document.getElementById('imagePreview');
                const label = document.querySelector('label[for="photo_profile"]');
                if (kasir.photo_profile) {
                    preview.src = '../../assets/images/profile/' + kasir.photo_profile;
                    preview.classList.remove('hidden');
        ...
        function deleteKasir(id) {
            ...
            <div style="text-align: center; margin: 20px 0;">
                ...
                <p style="font-size: 1.1rem; margin-bottom: 10px;">Apakah Anda yakin ingin menghapus kasir:</p>
                <p style="font-size: 1.3rem; font-weight: 600; color: #333;">${kasir ? kasir.nama : 'ini'}</p>
        ...
        document.addEventListener('DOMContentLoaded', function() {
            // Close modal when clicking outside
            document.getElementById('kasirModal').addEventListener('click', function(e) {
                if (e.target === this) {
                    closeModal();
                }
            });
